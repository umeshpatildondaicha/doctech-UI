...
export class ExerciseCardComponent {
  ...
  get footerLayout(): 'row' | 'column' {
    return this.variant === 'compact' ? 'column' : 'row';
  }
  ...
  get footerActions(): ListingCardAction[] {
    ...
    if (this.allowSelection) {
      actions.push({
        ...
        style: this.isSelected ? 'warn' : 'primary',
    ...
    if (this.showActions) {
      actions.push(
        { id: 'view', label: 'View', icon: 'visibility', style: 'neutral', tooltip: 'View exercise' },
        { id: 'edit', label: 'Edit', icon: 'edit', style: 'neutral', tooltip: 'Edit exercise' }
      );
      if (!this.allowSelection) {
        actions.push({ id: 'delete', label: 'Delete', icon: 'delete', style: 'neutral', tooltip: 'Delete exercise' });
      }
    }
